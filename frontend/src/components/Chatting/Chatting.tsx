...
export default function Chatting({ sender, receiver }: ChattingProps) {
    ...
    useEffect(() => {
        ...
        async function fetchUserIds() {
            try {
                const senderRes = await axios.post("http://localhost:8000/api/getId", { email: sender });
                const receiverRes = await axios.post("http://localhost:8000/api/getId", { email: receiver });
        ...
        async function fetchMessages() {
            try {
                const res = await axios.post("http://localhost:8000/api/conversation", { to: receiver, from: sender });
